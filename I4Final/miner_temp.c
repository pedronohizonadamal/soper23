...
int main (int argc, char **argv) {
    ...
    snprintf(filename, sizeof(filename), "log_%d", getpid());

    if ((block_register = fopen(filename, "w")) == NULL ) {
    ...
    // Proceso registrador
    if (pid == 0) {
        close(pipeToRegister[1]);
        while (1) {

            strcpy(file_output, "");

            if (read(pipeToRegister[0],&block,sizeof(block)) == 0){
                break;
            }
            ...
            if (block.end == true) {
                fclose(block_register);
                break;
            }
        }
        close(pipeToRegister[0]);
        exit(EXIT_SUCCESS);
    }
